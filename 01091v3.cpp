...
ll scan(ll l, ll r, vector<ll> raw){
    ...
    for(ll i = l; i < r; i++){
        ...
        bool validNarrow =  (double) max(raw[i]*2, Max)/min(raw[i]*2, Max) <= (double) 105/95;
        bool validWide = (double) max(raw[i], Max)/min(raw[i], Max) <= (double) 105/95;
        ...
        if(temp.length()==5){
            ...
            if(i < r-5){
                ...
                validNarrow =  (double) max(raw[i+1]*2, Max)/min(raw[i+1]*2, Max) <= (double) 105/95;
                if(!validNarrow){
                    final.push_back(bar);
                    // cout << raw[i+1] << " " << Min << "\n";
                    // cout << "Narrow invalid!\n";
...
int main(){
    ...
    while(cin >> n && n){
        ...
        for(ll i = 0; i < n; i++){
            bool validNarrow =  (double) max(raw[i]*2, Max)/min(raw[i]*2, Max) <= (double) 105/95;
            if(validNarrow)  Max = max(Max, raw[i]*2);
        }
