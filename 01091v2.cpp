...
ll scan(ll l, ll r, vector<ll> raw){
    ...
    for(ll i = l; i < r; i++){
        ...
        bool validNarrow =  (double) raw[i] >=(double) (intX*0.95) && (double) raw[i]<=(double) (intX*1.05);
        bool validWide = (double) raw[i]>=(double) (intX*1.9) && (double) raw[i]<=(double) (intX*2.1);
        ...
        if(temp.length()==5){
            ...
            if(i < r-5){
                ...
                validNarrow =  (double) raw[i+1] >=(double) (intX*0.95) && (double) raw[i+1]<=(double) (intX*1.05);
                if(!validNarrow){
                    final.push_back(bar);
                    // cout << raw[i+1] << " " << Min << "\n";
                    // cout << "Narrow invalid!\n";
