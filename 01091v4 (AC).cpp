...
ll scan(ll l, ll r, vector<ll> raw){
    ...
    ll nMax = LLONG_MIN, nMin = LLONG_MAX;

    for(ll i = l; i < r; i++){
        // cout << (double)Max/(raw[i]*2) << "\n";
        
        bool validNarrow =  max(raw[i]*2, Max)*95 <= 105*min(raw[i]*2, Max); // must be Max(x,y)
        bool validWide = max(raw[i], Max)*95 <= 105*min(raw[i], Max);
        ...
        if(temp.length()==5){
            ...
            if(i < r-5){
                ...
                validNarrow =  max(raw[i+1]*2, Max)*95 <= 105*min(raw[i+1]*2, Max); // must be Max(x,y)
                if(!validNarrow){
                    final.push_back(bar);
                    // cout << raw[i+1] << " " << Min << "\n";
                    // cout << "Narrow invalid!\n";
    ...
    for(ll i = l; i < r; i++){ // final checking
        nMax = max(nMax, raw[i]);
        nMin = min(nMin, raw[i]);  
    } 
    ...
    if(bar[0] != 'S' || bar[bar.length()-1] != 'S' || stcnt != 2 || (95*nMax > 105*nMin)){
        final.push_back(bar);
        return -1;
    }
    ...
    if(! checkC(bar[bar.size()-2])) return 0;
...
int main(){
    ...
    while(cin >> n && n){
        ...
        vector<ll> raw,barr; Max = LLONG_MIN, Min = LLONG_MAX;
        cout << "Case " << cs++ << ": ";
        for(ll i = 0; i < n; i++){
            ll temp; cin >> temp;
            barr.push_back(temp);
