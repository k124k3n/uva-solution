...
class SegmentTree{
    public:
        ...
        void prop(ll now, ll l, ll r, vector<ll> &lazy, vector<ll> &st, vector<ll> &x, vector<ll> &y){ // 2
            if(lazy[now]!=-1){
                st[now] = ((st[now]%mod)+(y[now]%mod)((x[now]%mod)*(lazy[now]%mod) % mod)) % mod;
                
                ll mid = (l+r)/2;

                if(l != r){
                    prop(now*2, l, mid, lazy, st, x, y);
                    prop(now*2+1, mid+1, r, lazy, st, x, y);
        ...
        void update(ll now, ll l, ll r, ll i, ll j, vector<ll> &lazy, vector<ll> &st, vector<ll> &x, vector<ll> &y, ll val){
            if(i > j) return;
            if(i > r || l > j) return;
            ...
            update(now, l, mid, i, min(mid, j), lazy, st, x, y, val);
            update(now, mid+1, r, max(mid+1, i), j, lazy, st, x, y, val);
            st[now] = ((st[now*2] % mod) + (st[now*2+1] % mod)) % mod;
            cout << l << " " << r << " = " << st[now] << "\n";
        }
...
int main(){
    ...
    while(q--){
        ...
        // update double
        cout << "AB\n\n";
        st.update(1, 0, (n-1), l, r, abl, ab, b, x); // (A+x)(B+y)= AB + Bx
        st.update(1, 0, (n-1), l, r, abl, ab, a, y); // A(B+y) = AB + Ay
